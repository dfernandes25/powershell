...
function Get-InstalledSoftware {
    ...
    Function Remove-Package() {
        ...
        if ( $package.ComputerName -eq $env:COMPUTERNAME ) {
            Write-Verbose -Message "Removing `"$($package.DisplayName)`" ..."
                ...
                if ( $PSCmdlet.ShouldProcess( $package.DisplayName , 'Remove package' ) ) {
                    ...
                    if ( $silent ) {
                        if ( $executable -match '^msiexec\.exe$' -or $executable -match '^msiexec$' -or $executable -match '[^a-z0-9_]msiexec\.exe$' -or $executable -match '[^a-z0-9_]msiexec$' ) {
                            ...
                            Write-Warning -Message "Don't know how to run silent uninstall for package `"$($package.DisplayName)`", uninstaller `"$executable`""
                        }
                    }
                    if ( ! [string]::IsNullOrEmpty( $arguments ) ) {
                        $processArguments.Add( 'ArgumentList' , $arguments )
                    }
                    Write-Verbose -Message "Running $executable `"$arguments`" for $($package.DisplayName) ..."
...
$rptdir = 'C:\Scripts\reports'

if (!(Test-Path -Path $rptdir))
{ mkdir $rptdir -Force }

Get-InstalledSoftware -exportcsv "$rptdir\$cname-installed-software.csv"
...
function Send-FTP { 
    ...
    foreach ($item in (Get-ChildItem -Path "$rptdir")) {
